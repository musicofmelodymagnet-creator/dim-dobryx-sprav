...
- **Ощадбанк** — IBAN з кнопкою копіювання, QR-код (збільшується при кліку), PDF реквізити, кнопка "Відкрити в додатку" (bank.gov.ua)
- **Монобанк** — QR-код з лайтбоксом, кнопка "Відкрити в додатку" (bank.gov.ua)
- Обидва QR-коди ведуть на офіційні платіжні посилання НБУ-формату — відкривають будь-який банківський застосунок
...
└── docs/
    ...
    ├── Vipiska.pdf         # Виписка
    ├── Vityag.pdf          # Витяг
    ├── oschadbank-rekv.pdf # Реквізити рахунку Ощадбанк
    ├── oschadbank-doc1.pdf # Довідка про рахунок
    ├── oschadbank-doc2.pdf # Додаткові документи
    ├── oschadbank-qr.png   # QR-код для переказу (Ощадбанк)
    └── monobank-qr.jpg     # QR-код для переказу (Монобанк)
```
